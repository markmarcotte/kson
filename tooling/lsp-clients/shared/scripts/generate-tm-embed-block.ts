...
interface LanguageConfig {
    ...
    contentName: string;
    /**
     * Strip the block's own indentation before the injected grammar sees a line.
     * Markdown needs it: four leading spaces read as a code block. Not universal --
     * stripping breaks grammars anchored to the real line start, such as ruby heredocs.
     */
    stripIndent?: boolean;
}
 ...
 *
 * `begin` captures the first content line's indent; `while` matches it literally on
 * each later line, so content keeps any indentation of its own.
 *
 * `while` also terminates the block: refusing the closing delimiter makes it fail,
 * which discards the injected grammar and lets the block's own `end` match. The
 * blank-line alternative stops an empty line re-measuring the indent.
 *
 * @param delimiter - only this block's own pair is excluded; the other stays content
 */
...
function generateEmbedPattern(lang?: LanguageConfig, delimiter?: Delimiter) {
    ...
    return {
        ...
        name,
        // `[ \t\r]`, not `\s`: `\s` would match the newline that ends the tag.
        ...
        // `^` ties with the embedded grammar's column-0 rules; `end` is tried first, so it wins.
        // `\1\1` rather than a literal: one rule serves both delimiters, so a `$` block must not
        // close on `%%`. Non-greedy per docs/readme.md -- the first closer on the line ends it.
        end: "^(.*?)(\\1\\1)",
        endCaptures: {
            "1": {
